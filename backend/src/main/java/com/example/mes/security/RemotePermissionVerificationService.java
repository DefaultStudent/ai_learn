...
public class RemotePermissionVerificationService implements PermissionVerificationService {
    ...
    public VerifiedLogin verify(LoginRequest request) {
        ...
        Authentication authentication = new UsernamePasswordAuthenticationToken(response.username(), null, List.of(new SimpleGrantedAuthority("ROLE_" + role.name())));
        return new VerifiedLogin(authentication, LoginResponse.of(response.id(), response.username(), response.displayName(), role, response.department()));
    }

    /** @param id 用户主键 @param username 登录账号 @param displayName 显示名称 @param role 角色 @param department 部门 */
    private record RemoteLoginResponse(Long id, String username, String displayName, String role, String department) { }
}
